...
func New(address string, logger *log.Logger, store store.Store) server.Server {
	return &seccyServer{
		address: address,
		logger:  logger,
		store:   store,
...
func (s *seccyServer) Set(_ context.Context, req *seccy.SetRequest) (*empty.Empty, error) {
	if req.Key == "" {
		return &empty.Empty{}, server.EmptyKey
	}
	if req.Value == "" {
		return &empty.Empty{}, server.EmptyValue
	}
...
func (s *seccyServer) ListKeys(_ *empty.Empty, stream seccy.Seccy_ListKeysServer) error {
	s.logger.Debug("Listing all keys")
